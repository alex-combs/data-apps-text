...
```{r ncwhitemom, echo=FALSE, fig.align='center', fig.cap="Race of a sample of pregnant women in North Carolina"}
...
We usually want to provide the *frequency* of the different levels in terms of proportion or percentage like in the table below, which is commonly referred to as a **frequency table**. Here, we see that 77% of the pregnant women in this sample are white.
...
To convey a possible association between two categorical variables without using a visualization, we can use a **contingency table**. Below is a contingency table for whether a mother in this sample was smoker and whether they had a premature birth.

```{r, echo=FALSE}
mytable <- table(ncbirths$habit, ncbirths$premie) 

mytable.1 <- addmargins(mytable) 
