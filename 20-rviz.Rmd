...
For example, the below code takes the variable `gdpPercap` from the dataset `gapminder` and maps it to the horizontal x axis of a chosen `geom`etric object called a histogram.
...
For example, you can't generate a scatter plot--`geom_point`--if you only define an `x` aesthetic. You must define an `x` and `y` aesthetic for a scatter plot. The figure below lists various aesthetic attributes. 

```{r, echo=FALSE}
include_graphics('_bookdown_files/ggaes.png')
```
...
**Exercise 1:**  In the `countyComplete` dataset, there is a variable named `bachelors` that measures the percent of the county population with a bachelor's degree between 2006-2010. Suppose we want to visualize the distribution of `bachelors` with a histogram. Generate a simple, default histogram (no optional code unless you want to add it).
```

```{r, include=FALSE}
ggplot(countyComplete, aes(x = bachelors)) +
  geom_histogram()
```
...
```{block, type='learncheck', purl=FALSE}
**Exercise 2:**  Generate a simple, default box plot for `bachelors` (no optional code unless you want to add it).
```
...
```{block, type='learncheck', purl=FALSE}
**Exercise 3:**  Suppose we want to visualize how many counties each state has. That is, we want to count how many rows belong to each `state` in the `countyComplete` data using a bar chart. Generate a bar chart that achieves this. Choose the type of bar chart you deem best.
```
...
```{block, type='learncheck', purl=FALSE}
**Exercise 4:**  Pick two variables in the `countyComplete` data and plot their relationship using a simple scatter plot.
```
...
```{block, type='learncheck', purl=FALSE}
**Exercise 5:**  Create a line graph for male prisoners who were sentenced by state.
```
